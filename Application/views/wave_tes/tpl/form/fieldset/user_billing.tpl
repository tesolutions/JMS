...
<div class="form-group row[{if $aErrors.oxuser__oxustid}] text-danger[{/if}]">
    ...
    <div class="col-lg-9">
        <input class="form-control" type="text" maxlength="255" name="invadr[oxuser__oxustid]" placeholder="Nur für Firmenkunden" id="invadr_oxuser__oxustid" value="[{if isset( $invadr.oxuser__oxustid )}][{$invadr.oxuser__oxustid}][{else}][{$oxcmp_user->oxuser__oxustid->value}][{/if}]"[{if $oView->isFieldRequired(oxuser__oxustid)}] required=""[{/if}]>
