    ...
    [{if $aUserAddresses|@count > 0}]
        ...
        [{/if}]
            ...
            [{block name="form_user_shipping_address_select"}]
                <div class="row dd-available-addresses" data-toggle="buttons" style="padding-bottom:20px;">
...
<div id="shippingAddressForm" style="margin-top:20px;[{if $delivadr}]display: none;[{/if}]">
    <div class="form-group">
        <label class="control-label col-lg-3 req" for="sendtopackstation">Lieferung an</label>
        <div class="col-lg-3">
             <select name="sendtopackstation" class="form-control selectpicker bs-select-hidden" id="sendtopackstation" onchange="input_ID()">
                <option value="adress" selected="">Lieferadresse wie eingegeben</option>
                <option value="pack">DHL Packstation</option>
            </select>
        </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group[{if $aErrors.oxaddress__oxcompany}] oxInValid[{/if}]">
        <label id="companylabel" class="control-label col-lg-3[{if $oView->isFieldRequired(oxaddress__oxcompany)}] req[{/if}]" for="deladr_oxaddress__oxcompany">[{oxmultilang ident="COMPANY"}]</label>
        <div class="col-lg-9">
   ...
   <div class="form-group[{if $aErrors.oxaddress__oxstreet}] oxInValid[{/if}]">
       <label id="streetlabel" class="control-label col-xs-12 col-lg-3[{if $oView->isFieldRequired(oxaddress__oxstreet) || $oView->isFieldRequired(oxaddress__oxstreetnr)}] req[{/if}]">[{oxmultilang ident="STREET_AND_STREETNO"}]</label>
       <div class="col-xs-8 col-lg-6">
           <input class="form-control[{if $oView->isFieldRequired(oxaddress__oxstreet)}] js-oxValidate js-oxValidate_notEmpty[{/if}]" type="text" data-fieldsize="pair-xsmall" maxlength="255" name="deladr[oxaddress__oxstreet]" id="oxaddress__oxstreet" value="[{if isset( $deladr.oxaddress__oxstreet )}][{$deladr.oxaddress__oxstreet}][{else}][{$delivadr->oxaddress__oxstreet->value}][{/if}]"[{if $oView->isFieldRequired(oxaddress__oxstreet)}] required=""[{/if}]>
       </div>
                ...
                <select class="form-control[{if $oView->isFieldRequired(oxaddress__oxcountryid)}] js-oxValidate js-oxValidate_notEmpty[{/if}]" id="delCountrySelect" name="deladr[oxaddress__oxcountryid]"[{if $oView->isFieldRequired(oxaddress__oxcountryid)}] required=""[{/if}] autocomplete="shipping country">
                    ...
                    [{foreach from=$oViewConf->getCountryList() item=country key=country_id}]
                        ...
                        [{if !$blCountrySelected}]
                            ...
                            [{elseif (!isset($invadr.oxuser__oxcountryid) && $country->oxcountry__oxid->value == 'a7c40f631fc920687.20179984' )}]
                                [{assign var="sCountrySelect" value="selected"}]
                            [{/if}]
                        [{/if}]
...
<script type="text/javascript">
    function input_ID(){
        let tes_pack = document.getElementById("sendtopackstation").value;
        if(tes_pack === "pack"){
            document.getElementById('companylabel').classList.add('req');
            document.getElementById('companylabel').innerHTML = 'Postnummer';
            document.getElementById('streetlabel').innerHTML = 'Packstation Nummer';
            document.getElementById('oxaddress__oxstreet').value = 'Packstation';
            document.getElementById('oxaddress__oxstreet').readOnly = true;
        }
        else if(tes_pack === "adress"){
            document.getElementById('companylabel').classList.remove('req');
            document.getElementById('companylabel').innerHTML = 'Firma';
            document.getElementById('streetlabel').innerHTML = 'Straße, Hausnummer';
            document.getElementById('oxaddress__oxstreet').readOnly = false;
        }
    }
</script>