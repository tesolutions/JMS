    ...
    [{block name="checkout_basket_main"}]
        ...
        [{if $oView->isLowOrderPrice()}]
            [{block name="checkout_basket_loworderprice_top"}]
                <div class="alert alert-info" style="font-weight: 700;color: red;text-align: right;">[{oxmultilang ident="MIN_ORDER_PRICE"}] [{$oView->getMinOrderPrice()}] [{$currency->sign}]</div>
        ...
        [{if !$oxcmp_basket->getProductsCount()}]
            [{block name="checkout_basket_emptyshippingcart"}]
                <div class="spacer"></div>
                <div class="alert alert-danger" id="empty-basket-warning" style="font-weight: 700;color: red;">
                        ...
                        [{if !$oView->isLowOrderPrice()}]
                            [{block name="basket_btn_next_bottom"}]
                                <form action="[{$oViewConf->getSslSelfLink()}]" method="post" class="float-right">
                                    ...
                                    [{if $oxcmp_shop->oxshops__urlaub->value == "0"}]
                                        <button type="submit" class="btn btn-primary submitButton largeButton nextStep pull-right">[{oxmultilang ident="CONTINUE_TO_NEXT_STEP"}] <i class="fa fa-caret-right"></i></button>
                                    [{/if}]
                                </form>
                            [{/block}]
                        [{/if}]
