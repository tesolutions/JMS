...
[{capture append="oxidBlock_content"}]
    [{block name="page_list_listhead"}]
        ...
        [{if $actCategory->oxcategories__oxlongdesc->value && $oPageNavigation->actPage == 1}]
            <p>[{oxeval var=$actCategory->oxcategories__oxlongdesc}]</p>
            <hr/>
        [{/if}]
                ...
                <div class="row">
                    [{foreach from=$oView->getSubCatList() item=category name=MoreSubCat}]
                        [{if $category->getContentCats()}]
                            ...
                            [{/foreach}]
                        [{/if}]
                        [{if $iSubCategoriesCount%6 == 0 and $iSubCategoriesCount neq 0}]
                            </div><div class="row">
                        [{/if}]
...
[{/capture}]
[{include file="layout/page.tpl" tree_path=$oView->getTreePath() sidebar="Left"}]