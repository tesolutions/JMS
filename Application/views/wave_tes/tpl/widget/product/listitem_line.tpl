                ...
                <div class="functions text-right">
                    [{block name="widget_product_listitem_line_price"}]
                        ...
                        [{if $tprice && $tprice->getBruttoPrice() > $price->getBruttoPrice()}]
                            <span class="oldPrice text-muted">
                                Unser bisheriger Preis <del>[{$product->getFTPrice()}] [{$currency->sign}]</del>
                            </span>
                        [{/if}]
                        ...
                        [{if $oUnitPrice}]
                            <span id="productPricePerUnit_[{$testid}]" class="pricePerUnit text-nowrap" style="font-size: 12px"><br>[{$product->oxarticles__oxunitquantity->value}] [{$product->getUnitName()}] | [{oxprice price=$oUnitPrice currency=$currency}]/[{$product->getUnitName()}]</span>
                        [{/if}]
                        [{/oxhasrights}]
                    [{/block}]
                        <div class="ampel_list">
                            <div class="ampel_list_ig text-right" style="font-size:12px;margin-top: 2px;">
                    ...
                    [{block name="widget_product_listitem_line_tobasket"}]
                        [{if $blShowToBasket}]
                            ...
                            [{/oxhasrights}]
                        [{else}]
                            <a class="btn btn-primary btn-block" href="[{$_productLink}]" style="margin-top: 10px;">[{oxmultilang ident="MORE_INFO"}]</a>

                            [{if $removeFunction && (($owishid && ($owishid==$oxcmp_user->oxuser__oxid->value)) || (($wishid==$oxcmp_user->oxuser__oxid->value)) || $recommid)}]
                                <button triggerForm="remove_[{$removeFunction}][{$testid}]" type="submit" class="btn btn-danger btn-block removeButton" style="margin-top: 5px;">
